...
DF=0.3 #Dropout Freq
WT=3.0 #weight for VH error in class_weight
L1=1500 #Number of units in Hidden Layer 1
L2=1500 #Number of units in Hidden Layer 2
L3=1500 #Number of units in Hidden Layer 3
...
modelo.add(BatchNormalization()) #Evita loss nan
modelo.add(Dropout(DF))
modelo.add(Dense(units=L2,name='camada_escondida_2', activation='relu'))
modelo.add(BatchNormalization())
modelo.add(Dropout(DF))
modelo.add(Dense(units=L3,name='camada_escondida_3', activation='sigmoid'))
modelo.add(BatchNormalization())
modelo.add(Dropout(DF))
...
custom_adam = tf.keras.optimizers.Adam(learning_rate=LR, clipnorm=1.0)  # Customizando para definir a learning_rate
...
modelo.compile(optimizer = custom_adam,
               ...
               metrics = ['accuracy', keras.metrics.Precision(), tf.keras.metrics.Recall()])
...
#Salvar modelo
sv_pth='BS_'+str(BS)+'_LR_'+str(LR)+'_DF_'+str(DF)+'_WT_'+str(WT)+'_'+str(L1)+'_'+str(L2)+'_'+str(L3)
